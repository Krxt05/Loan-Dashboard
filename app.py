...
<style>
    ...
    /* 1. การ์ดข้อมูลทั่วไป (Metric Card) */
    .metric-card {{
        ...
        height: 100%;
        transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); /* Animation นุ่มนวล */
        cursor: default;
    }}

    /* เมื่อเอาเมาส์ชี้การ์ด */
    .metric-card:hover {{
        transform: translateY(-5px) scale(1.05); /* ลอยขึ้นและขยาย 5% */
        box-shadow: 0px 15px 30px rgba(0,0,0,0.15); /* เงาฟุ้งขึ้น */
        border-color: {THEME['Blue']}; /* เปลี่ยนขอบเป็นสีฟ้า */
        z-index: 10;
    }}
    ...
    .tracker-card:hover {{
        ...
        box-shadow: 0px 10px 20px rgba(255, 193, 7, 0.4);
    }}
    ...
    div[data-testid="stSelectbox"] *,
    div[data-testid="stDateInput"] * {{ cursor: pointer !important; }}
    div[data-testid="stSelectbox"] input,
    div[data-testid="stDateInput"] input {{ caret-color: transparent !important; cursor: pointer !important; }}
...
if app_mode == "📊 ภาพรวมทุกเดือน (Summary)":
    ...
    if not df.empty:
        try:
            ...
            def card(label, value, text_color):
                # ใช้ class metric-card เพื่อให้มี Animation
                st.markdown(f"""
                <div class="metric-card">
                    <div class="metric-label">{label}</div>
                    <div class="metric-value" style="color: {text_color};">{value}</div>
            ...
            with zone1_col:
                ...
                # ใช้ class hero-card เพื่อให้มี Animation
                st.markdown(f"""
            ...
            with zone2_col:
                ...
                if len(df) > 2:
                    ...
                    try:
                        ...
                        # ใช้ class tracker-card เพื่อให้มี Animation
                        st.markdown(f"""
