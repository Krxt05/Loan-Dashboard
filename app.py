...
import pytz # อย่าลืมใส่ pytz ใน requirements.txt ด้วยนะครับ
...
<style>
    ...
    div[data-testid="stSelectbox"]:hover {{
        box-shadow: 0px 6px 25px rgba(39, 174, 96, 0.5);
...
COL_IDX = {
    ...
    "interest": 9, "status": 12, "actual_date": 5, "actual_interest": 15
}
...
if app_mode == "📊 ภาพรวมทุกเดือน (Summary)":
    ...
    if all_months_data:
        ...
        with c1: card("💰 วงเงินล่าสุด", fmt(latest_balance), THEME["Text"])
        with c2: card("💵 กำไรสะสม", fmt(total_profit), THEME["Mint"])
        with c3: card("💸 เบิกสะสม", fmt(total_withdraw), "#F5B041")
        with c4: card("🥀 โดนบิดรวม", fmt(total_scam), THEME["Pink"])
        with c5: card("🌱 กำไรสุทธิรวม", fmt(total_net), THEME["Blue"])
    ...
    with col_title:
        ...
        # --- DEBUG TIMEZONE (เช็คเวลา Server) ---
        try:
            th_tz = pytz.timezone('Asia/Bangkok')
            now_th = datetime.now(th_tz)
            # ถ้าอยากซ่อนบรรทัดนี้ ให้ใส่เครื่องหมาย # ข้างหน้า st.caption
            st.caption(f"🕒 Server Time (TH): {now_th.strftime('%d/%m/%Y %H:%M:%S')}")
        except Exception as e:
            st.caption("⚠️ pytz not installed correctly")
        # ----------------------------------------
    ...
    if not df.empty:
        try:
            ...
            def card(label, value, text_color):
                st.markdown(f"""<div class="metric-card"><div class="metric-label">{label}</div><div class="metric-value" style="color: {text_color};">{value}</div></div>""", unsafe_allow_html=True)
            ...
            with zone2_col:
                ...
                if len(df) > 2:
                    ...
                    try:
                        ...
                        sum_princ = pd.to_numeric(due_target['principal'].astype(str).str.replace(',', ''), errors='coerce').sum()
                        sum_int = pd.to_numeric(due_target['interest'].astype(str).str.replace(',', ''), errors='coerce').sum()
                        ...
                        if not due_target.empty:
                            ...
                            st.dataframe(due_target[['id', 'name', 'principal', 'interest']], hide_index=True, use_container_width=True)
                        else:
            ...
            with col_bar:
                ...
                fig_bar = px.bar(bar_df, x='รายการ', y='บาท', text_auto='.2s', color='รายการ', color_discrete_sequence=colors)
                fig_bar.update_layout(showlegend=False, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#2C3E50")
                st.plotly_chart(fig_bar, use_container_width=True)

            with col_pie:
                cost = balance - profit
                pie_df = pd.DataFrame({'Type': ['ต้นทุน', 'กำไร'], 'Value': [cost, profit]})
                fig_pie = px.pie(pie_df, values='Value', names='Type', color_discrete_sequence=[THEME["Grey"], THEME["Mint"]], hole=0.6)
                fig_pie.update_layout(paper_bgcolor="rgba(0,0,0,0)", showlegend=True, legend=dict(orientation="h", y=-0.2))
            ...
            if len(df) > 2:
                ...
                try:
                    ...
                    due_data['Amount'] = pd.to_numeric(cal_df.iloc[:, COL_IDX['interest']].astype(str).str.replace(',', ''), errors='coerce')
                    
                    actual_data = pd.DataFrame()
                    actual_data['Date'] = cal_df.iloc[:, COL_IDX['actual_date']].apply(parse_thai_date)
                    actual_data['Amount'] = pd.to_numeric(cal_df.iloc[:, COL_IDX['actual_interest']].astype(str).str.replace(',', ''), errors='coerce')
                    ...
                    if not final_chart_df.empty:
                        fig_cal = px.bar(final_chart_df, x='Date', y='Amount', color='Type',
                                         ...
                                         color_discrete_map={'คาดการณ์ (Due)': THEME['Mint'], 'ชำระจริง (Actual)': THEME['Blue']},
                                         text_auto='.2s')
                        fig_cal.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#2C3E50")
